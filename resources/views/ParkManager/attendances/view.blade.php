...
Auth::user()->type == 'Technicien' ||Auth::user()->type=='Utilisateur'||Auth::user()->type=='Gestionnaire Sup'  )
				...
				<div class="page-header">
					<div class="row">
						<div class="col-md-6 col-sm-12">
							<div class="title">
								<h4>Liste de pointage pour   {{$staff->name}} {{$staff->last_name}}:</h4>
                     		</div>
					...
					<div class="card-box mb-30">
						...
						<div class="pb-20">
							...
							<table class="table nowrap hover data-table-export">
                             ...
                             @if($attendance->id==$hour->attendance_id)
                                    <tr>
                                    <td>{{$attendance->id}}</td>
                                    <td> {{$attendance->arrived_at_date}} </td>
                                    <td> {{$attendance->arrived_at_time}} </td>
                                    <td> {{$attendance->left_at_date}} </td>
                                    <td> {{$attendance->left_at_time}} </td>
                                    <td> {{$attendance->observation}} </td>
                                    <td> {{$attendance->arrived_note}} </td>
                                    <td> {{$attendance->left_note}} </td>
                                    <td> {{$hour->early_hours/100}} </td>
                                    <td> {{$hour->late_hours/100}} </td>
