                            ...
                            <div class="rounded-lg bg-white shadow-lg md:shadow-xl relative overflow-hidden">
                                <div class="px-3 pt-8 pb-10 text-center relative z-10">
                                    <h1 class="text-sm uppercase text-gray-500 leading-tight" style="font-size: 2.5em;">
                                        Le véhicule a fonctionné pendant {{ $specificDays }} Jours en
                                        {{ $yearVA }}
                                    </h1>
                                    <h3 class="text-3xl text-gray-700 font-semibold leading-tight my-3">
                                        ({{ $VA }}% de l'année {{ $yearVA }})</h3>
            ...
            <div class="card-box pd-20 height-100-p mb-30">
                <div class="row align-items-center">
                    ...
                    <div class="col-md-12">
                        ...
                        <main>
                            <h4 class="font-20 weight-500 mb-10 text-capitalize text-orange">Conducteur
                            </h4>
                                        ...
                                        @foreach ($participants as $driver)
                                            ...
                                            <tr>
                                                ...
                                                @foreach ($specificTimes as $specificTime)
                                                    @if ($driver->id == $specificTime->driver_id)
                                                    @php $a=new DateTime($specificTime->end_date)  @endphp
                                                    @php $b=new DateTime($specificTime->start_date) @endphp
                                                    @php $consume=$consume+($a->diff($b))->format('%a') @endphp

                                                    @endif
                                                @endforeach
                                                <td style="color: orange;">
                                                    @if ($specificDays != 0)
                                                        {{ number_format(((float) $consume / $specificDays) * 100, 2, '.', '') }}%
